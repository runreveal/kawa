...
func (s *Journald) recvLoop(ctx context.Context) error {
	...
	hwmFile, err := os.OpenFile("/tmp/flowd-journald-hwm", os.O_RDWR|os.O_CREATE, os.FileMode(0644))
	if err != nil {
		return err
	}
	defer hwmFile.Close()
...
func (s *Journald) Recv(ctx context.Context) (flow.Message[types.Event], func(), error) {
	select {
	case <-ctx.Done():
